...
FifoUrlList.prototype.insertIfNotExistsBulk = function (urls) {
  for (const url of urls) {
    this.insertIfNotExists(url);
  }
...
FifoUrlList.prototype.upsert = function (url) {
  const uniqueId = url.getUniqueId();

  return this.insertIfNotExists(url).then(() => {
    const currentIndex = this._listIndexesByUniqueId[uniqueId];
...
FifoUrlList.prototype.getNextUrl = function () {
  ...
  const item = this._list[this._nextIndex];
  this._nextIndex++;
